    ...
    if (!tablExist) {
      await queryInterface.createTable(
        'order_item',
        {
          ...
          order_id: { type: DataTypes.UUID, unique: true, allowNull: false, references: { model: 'order', key: 'id' } },
          product_item_id: { type: DataTypes.UUID, allowNull: false, references: { model: 'product_item', key: 'id' } },
          qty: { type: DataTypes.INTEGER({ unsigned: true }), allowNull: false },
          amount: { type: DataTypes.BIGINT({ unsigned: true }), allowNull: false },
